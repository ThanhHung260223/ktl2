...
namespace QLcuahangbanmaytinh
{
    public partial class frmhoadonban : Form
    {
        ...
        private void btnthem_Click(object sender, EventArgs e)
        {
            ...
            if (kq != null)
            {
                string sqlthem = "insert into Hoadonban values ('" + txtmahoadonban.Text + "','" + txtmanv.Text + "','" + cbomakh.SelectedValue + "','" + cbomamaytinh.SelectedValue + "','" + txtKho.Text + "','" + txtsoluong.Text + "','" + mtbNgayban.Text + "','" + txtdiachi.Text + "','" + txtsodienthoai.Text + "','" + txtdongia.Text + "','" + txtTongtien.Text + "')";
                co.ThucThi(sqlthem);
        ...
        private void dgvDShdb_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            txtsoluong.Text = dgvDShdb.Rows[i].Cells[5].Value.ToString().Trim();
            txtKho.Text = dgvDShdb.Rows[i].Cells[4].Value.ToString().Trim();
            mtbNgayban.Text = dgvDShdb.Rows[i].Cells[6].Value.ToString().Trim();  
            txtdiachi.Text = dgvDShdb.Rows[i].Cells[7].Value.ToString().Trim();
            txtsodienthoai.Text = dgvDShdb.Rows[i].Cells[8].Value.ToString().Trim();
            txtdongia.Text = dgvDShdb.Rows[i].Cells[9].Value.ToString().Trim();
            txtTongtien.Text = (double.Parse(txtdongia.Text) * int.Parse(txtsoluong.Text)).ToString();
            textBox1.Text = dgvDShdb.Rows[i].Cells[5].Value.ToString().Trim();
        }

        private void btnsua_Click(object sender, EventArgs e)
        {
            int soluong = int.Parse(txtsoluong.Text), soluongcu = int.Parse(textBox1.Text);
            double dongia = double.Parse(txtdongia.Text);
            double tongtien = soluong * dongia;
            txtTongtien.Text = tongtien.ToString();
            co.KetNoi();
            string sqlsua = "update Hoadonban set MaHDB='" + txtmahoadonban.Text + "',MaNV='" + txtmanv.Text + 
                "',MaKH='" + cbomakh.SelectedValue + "',Soluong='" + txtsoluong.Text + "',Ngayban='" + mtbNgayban.Text + "',Diachi='" + txtdiachi.Text + 
