...
namespace QLcuahangbanmaytinh
{
    public partial class frmtkthongtinmaytinh : Form
    {
        ...
        {
            co.KetNoi();
            dgvtkthongtinmaytinh.DataSource = co.GetData("select * from SanPham where TENMT like '" + txttktheoten.Text.Trim() + "%'");
        }

        private void btntktheoma_Click(object sender, EventArgs e)
        {
            co.KetNoi();
            dgvtkthongtinmaytinh.DataSource = co.GetData("select * from SanPham where MaMT = '" + txttktheoma.Text.Trim() + "'");
        }
